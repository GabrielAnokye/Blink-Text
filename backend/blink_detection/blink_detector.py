...
class BlinkDetector:
    ...
    def detect_blink(self, frame):
        # FaceMesh graph is not thread-safe. If another frame is being processed,
        # drop this frame to keep packet timestamps strictly increasing.
        ...
        try:
            try:
                ...
                # Recover from MediaPipe graph timestamp desync.
                if "Packet timestamp mismatch" in str(err):
                    print("[BlinkDetector] MediaPipe timestamp mismatch. Resetting FaceMesh graph.")
                    self._reset_face_mesh()
                    return None
                raise
